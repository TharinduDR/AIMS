...
train = train.rename(columns={'text': 'input_text', 'summary': 'target_text'})
test = test.rename(columns={'text': 'input_text', 'summary': 'target_text'})
...
model_args.output_dir = os.path.join("outputs_xlsum", model_representation)
model_args.best_model_dir = os.path.join("outputs_xlsum", model_representation, "best_model")
model_args.cache_dir = os.path.join("cache_dir_xlsum", model_representation)

model_args.wandb_project = "XLSum Summary Generation"
...
test.to_csv(os.path.join("outputs_xlsum", model_representation, "predictions.tsv"), sep='\t', encoding='utf-8', index=False)
